...
// recuperation des paramètres LDAP dans la BdD SE3
require('/var/www/se3/includes/param_db.inc.php');
...
$res=mysql_query($sql);
	...
	if(mysql_num_rows($res)==1){
		$arr_tmp=mysql_fetch_array($res);
		$se3_adminRdn=$arr_tmp[0];
	}
...
$res=mysql_query($sql);
	if(mysql_num_rows($res)==1){
		$arr_tmp=mysql_fetch_array($res);
		$se3_adminPw=$arr_tmp[0];
	}

$sql="SELECT value FROM params WHERE name='ldap_base_dn'";
$res=mysql_query($sql);
	if(mysql_num_rows($res)==1){
		$arr_tmp=mysql_fetch_array($res);
		$se3_ldap_base_dn=$arr_tmp[0];
	}

$sql="SELECT value FROM params WHERE name='peopleRdn'";
$res=mysql_query($sql);
	if(mysql_num_rows($res)==1){
		$arr_tmp=mysql_fetch_array($res);
		$se3_peopleRdn=$arr_tmp[0];
	}

$sql="SELECT value FROM params WHERE name='rightsRdn'";
$res=mysql_query($sql);
	if(mysql_num_rows($res)==1){
		$arr_tmp=mysql_fetch_array($res);
		$se3_rightsRdn=$arr_tmp[0];
	}

$sql="SELECT value FROM params WHERE name='ldap_server'";
$res=mysql_query($sql);
	if(mysql_num_rows($res)==1){
		$arr_tmp=mysql_fetch_array($res);
		$se3_ldap_server=$arr_tmp[0];
	}
...
$sql="SELECT value FROM params WHERE name='ldap_port'";
$res=mysql_query($sql);
	if(mysql_num_rows($res)==1){
		$arr_tmp=mysql_fetch_array($res);
		$se3_ldap_port=$arr_tmp[0];
	}

$sql="SELECT value FROM params WHERE name='yala_bind'";
$res=mysql_query($sql);
	if(mysql_num_rows($res)==1){
		$arr_tmp=mysql_fetch_array($res);
		$se3_yala_bind=$arr_tmp[0];
	}
...
$servers->setValue('server','name','LDAP sambaedu3');
...
if ($se3_ip == $se3_ldap_server)
{
	$servers->setValue('server','host','ldap://localhost');
} else {
	$servers->setValue('server','host',"ldap://$se3_ldap_server");
	// ce cas n'a pas été testé !! 
}


/* The port your LDAP server listens on (no quotes). 389 is standard. */
$servers->setValue('server','port',$se3_ldap_port);

/* Array of base DNs of your LDAP server. Leave this blank to have phpLDAPadmin
   auto-detect it for you. */
$servers->setValue('server','base',array($se3_ldap_base_dn));
...
// verification des droits se3_is_admin ou Annu_is_admin
include_once('/var/www/se3/includes/functions.inc.php');
$userlogin=isauth();

if ($userlogin =='') {
	echo "Cette page n'est pas accessible sans authentification...";
	exit;
} else {
	$requete1 = "ldapsearch -xLLL member='uid=$userlogin,$se3_peopleRdn,$se3_ldap_base_dn' dn|grep cn=se3_is_admin|wc -l";
	$rights1 = exec($requete1);

	$requete2 = "ldapsearch -xLLL member='uid=$userlogin,$se3_peopleRdn,$se3_ldap_base_dn' dn|grep cn=Annu_is_admin|wc -l";
	$rights2 = exec($requete2);

	if ((!$rights1) and (!$rights2)) {
		$servers->SetValue('login','bind_pass','');
		echo "Vous n'avez pas les droits suffisants pour afficher cette page...";
		exit;
	} else {
		$servers->setValue('login','bind_id',$se3_adminRdn.",".$se3_ldap_base_dn);
		$servers->setValue('login','bind_pass',$se3_adminPw);
	}
}
...
/* Specify true If you want phpLDAPadmin to not display or permit any
 ...
 $servers->setValue('server','read_only', !$se3_yala_bind);
